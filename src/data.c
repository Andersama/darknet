...
box_label* read_boxes(char* filename, int* n)
{
    ...
    line_count += 1; // file always has at least one line because EOF counts

    boxes = (box_label*)xcalloc(line_count, sizeof(box_label));
...
box_label* read_boxes_cxx(char* filename, int* n, char** labels, int classes) {
    ...
    if (!file) {
        ...
        // previous api would've gaurunteed an allocation, we'll do the same
        boxes = (box_label*)xcalloc(1, sizeof(box_label));
        *n = 0;
        return boxes;
    }
    ...
    int64_t max_label_length = 0;
    char* label_buf = (char*)xcalloc(32, sizeof(char));
    ...
    while (fscanf(file, "%f %f %f %f ", &x, &y, &w, &h) == 4) {
        ...
        do
        {
            ...
            {
                break; //fscanf will skip leading whitespace so we don't have to, we can break here
            }
            else
            {
                ++label_length;
                if (label_length > max_label_length) {
                    max_label_length = (max_label_length * 2);
                    label_buf = (char*)xrecalloc(label_buf, max_label_length * sizeof(char));
                }
            }
            label_buf[label_length - 1] = character;
        } while (character != EOF);
        ...
        for (int i = 0; i < classes; i++) {
            char* label = labels[i];
            int character_matched = 0;
            int c = 0;
            for (; c < label_length; c++) {
                character_matched = (label[c] == label_buf[c]);
                if (character_matched == 0) {
                    break;
                }
                if (label[c] == 0 || label_buf[c] == 0) //could use goto to jump to outer loop
                    break;
            }
            if (!character_matched)
                continue;
            // if the next character to test terminated the string, it's a match
            if (label[c] == 0) {
                boxes[count].id = i;
                label_matched = 1;
                break;
...
void fill_truth_region_cxx(char* path, float* truth, char** labels, int classes, int num_boxes, int flip, float dx, float dy, float sx, float sy)
{
    char labelpath[4096];
    replace_image_to_label_cxx(path, labelpath);

    int count = 0;
    box_label* boxes = read_boxes_cxx(labelpath, &count, labels, classes);
    randomize_boxes(boxes, count);
    correct_boxes(boxes, count, dx, dy, sx, sy, flip);
    float x, y, w, h;
    int id;
...
int fill_truth_detection_cxx(const char* path, char **labels, int num_boxes, int truth_size, float* truth, int classes, int flip, float dx, float dy, float sx, float sy,
    int net_w, int net_h)
{
    char labelpath[4096];
    replace_image_to_label_cxx(path, labelpath);
...
data load_data_region_cxx(int n, char** paths, char** labels, int m, int w, int h, int size, int classes, float jitter, float hue, float saturation, float exposure)
{
    char** random_paths = get_random_paths(paths, n, m);
    int i;
    data d = { 0 };
    ...
    for (i = 0; i < n; ++i) {
        ...
        fill_truth_region_cxx(random_paths[i], d.y.vals[i], labels, classes, size, flip, dx, dy, 1. / sx, 1. / sy);
...
data load_data_detection_cxx(int n, char** paths, char** labels, int m, int w, int h, int c, int boxes, int truth_size, int classes, int use_flip, int gaussian_noise, int use_blur, int use_mixup,
    float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int contrastive_color, int show_imgs)
{
    const int random_index = random_gen();
    c = c ? c : 3;
    ...
    for (i_mixup = 0; i_mixup <= mixup; i_mixup++) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            fill_truth_detection_cxx(filename, labels, boxes, truth_size, truth, classes, flip, dx, dy, 1. / sx, 1. / sy, w, h);
...
void *load_thread(void *ptr)
{
    ...
    else if (a.type == REGION_DATA_CXX) {
        *a.d = load_data_region_cxx(a.n, a.paths, a.labels, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
    }
    ...
    else if (a.type == DETECTION_DATA_CXX) {
        *a.d = load_data_detection_cxx(a.n, a.paths, a.labels, a.m, a.w, a.h, a.c, a.num_boxes, a.truth_size, a.classes, a.flip, a.gaussian_noise, a.blur, a.mixup, a.jitter, a.resize,
            a.hue, a.saturation, a.exposure, a.mini_batch, a.track, a.augment_speed, a.letter_box, a.mosaic_bound, a.contrastive, a.contrastive_jit_flip, a.contrastive_color, a.show_imgs);
    }
